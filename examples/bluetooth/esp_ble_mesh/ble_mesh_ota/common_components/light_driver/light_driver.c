...
esp_err_t light_driver_init(light_driver_config_t *config)
{
    ...
    ESP_LOGI(TAG, "hue: %d, saturation: %d, value: %d, lightness: %d, color_temperature: %d",
             g_light_status.hue, g_light_status.saturation, g_light_status.value,
             g_light_status.lightness, g_light_status.color_temperature);
...
esp_err_t light_driver_set_ctl(uint8_t color_temperature, uint8_t lightness)
{
    LIGHT_PARAM_CHECK(lightness <= 100);
    LIGHT_PARAM_CHECK(color_temperature <= 100);

    esp_err_t ret = ESP_OK;
    uint8_t warm_tmp = color_temperature * lightness / 100;
    uint8_t cold_tmp = (100 - color_temperature) * lightness / 100;
    ...
    if (g_light_status.mode != MODE_CTL) {
...
{
    return light_driver_set_ctl(color_temperature, g_light_status.lightness);
}

esp_err_t light_driver_get_ctl(uint8_t *color_temperature, uint8_t *lightness)
{
...
{
    if (g_light_status.mode == MODE_CTL) {
        return light_driver_set_ctl(g_light_status.color_temperature, lightness);
    } else if (g_light_status.mode == MODE_HSL) {
...
esp_err_t light_driver_set_switch(bool on)
{
    ...
    if (!g_light_status.on) {
        ...
        switch (g_light_status.mode) {
            ...
            case MODE_CTL:
                g_light_status.lightness = (g_light_status.lightness) ? g_light_status.lightness : 100;
                ret = light_driver_set_ctl(g_light_status.color_temperature, g_light_status.lightness);
                LIGHT_ERROR_CHECK(ret < 0, ESP_FAIL, "light_driver_set_ctl, ret: %d", ret);
                break;
...
esp_err_t light_driver_fade_lightness(uint8_t lightness)
{
    esp_err_t ret = ESP_OK;
    g_fade_mode   = MODE_ON;
    ...
    if (g_light_status.mode == MODE_HSV) {
        ...
        if (lightness != 0) {
            ...
            uint8_t change_value    = lightness * 255 / 100 - max_color;
        ...
        g_light_status.value = lightness;
    ...
    } else if (g_light_status.mode == MODE_CTL) {
        uint8_t warm_tmp = 0;
        uint8_t cold_tmp = 0;
        fade_period_ms = LIGHT_FADE_PERIOD_MAX_MS * g_light_status.lightness / 100;

        if (lightness != 0) {
            uint8_t change_value = lightness - g_light_status.lightness;
            warm_tmp = g_light_status.color_temperature;
            cold_tmp = (lightness - g_light_status.color_temperature);
...
esp_err_t light_driver_fade_warm(uint8_t color_temperature)
{
    ...
    uint8_t warm_tmp =  color_temperature * g_light_status.lightness / 100;
    uint8_t cold_tmp = (100 - color_temperature) * g_light_status.lightness / 100;
...
esp_err_t light_driver_fade_stop(void)
{
    ...
    if (g_light_status.mode != MODE_CTL) {
        ...
        lightness         = (!color_temperature) ? cold_tmp : warm_tmp * 100 / color_temperature;

        g_light_status.lightness         = (g_fade_mode == MODE_OFF || g_fade_mode == MODE_ON) ? lightness : g_light_status.lightness;
        g_light_status.color_temperature = (g_fade_mode == MODE_CTL) ? color_temperature : g_light_status.color_temperature;
    }
